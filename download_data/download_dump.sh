...
case $1 in
	...
	*)
        ...
        LOCAL_FILENAME="/code/data/$DATE-$FILE"        #       save as        20191201-page.sql.gz
             LINK_NAME="/code/data/$FILE"              #     make link                 page.sql.gz
...
if [[ -f "$LOCAL_FILENAME" && -s "$LOCAL_FILENAME" ]]; then
	...
	echo -n "$PROTOCOL$DOMAIN/$WIKI/$DATE/$FULL_FILENAME"
